...
### 类加载器子系统作用
1. 类加载器子系统负责从文件系统或者网络中加载c1ass文件，c1ass文件在文件开头有特定的文件标识。
2. C1assLoader只负责c1ass文件的加载，至于它是否可以运行，则由 Execution Enginer决定
3. 加载的类信息存放于一块称为方法区的内存空间。除了类的信息外，方法区中还会存放运行时常量池信息，可能还包括字符串字面量和数字常量（这部分常量信息是
C1ass文件中常量池部分的内存映射）
	- 类加载器举例说明：
...
3. <font color=red size=3>***在内存中生成一个代表这个类的java.1ang.C1as对象***</font>，作为方法区这个类的各种数据的访问入口
